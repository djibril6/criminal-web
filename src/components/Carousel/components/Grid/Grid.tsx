...
interface IGridProps
  extends Partial<IBackgroundColor>,
...
export default class Grid extends Component<IGridProps, IGridState> {
  ...
  constructor(props: IGridProps) {
    ...
    this.state = {
      left: 0,
      current: this.props.setCurrent || 0,
      base: 100 * React.Children.count(props.children),
    };
    this.initialize();
  }
  ...
  getNumberOfPages() {
    return this.total < this.props.columns
      ? 1
      : 1 + this.total - this.props.columns;
  }
  ...
  onNext() {
    ...
    this.setState({
      left:
        temp <= -this.step * (this.getNumberOfPages() - 1)
          ? 0
          : temp - this.step,
      current:
        this.state.current >= this.getNumberOfPages() - 1
          ? 0
  ...
  render() {
    return (
      <>
        <div
          className="CarouselGlobal carouselGridContainer"
          ref={(el) => {
            if (el) {
              ...
              el.style.height = this.props.dimension?.height || '';
            }
          }}
        >
          <div
            ...
            ref={(el) => {
              if (el) {
                ...
                el.style.width =
                  Math.floor(this.state.base / (this.props.columns || 3)) + '%';
              }
            }}
          >
            {this.props.children}
                ...
                if (el) {
                  if (this.isCurrent(idx)) {
                    el.style.backgroundColor =
                      this.props.paginateCurrentColor || '#5191FA';
                  } else {
                    el.style.backgroundColor =
                      this.props.paginateColor || '#5E6D77';
                  }
                }
              }}
